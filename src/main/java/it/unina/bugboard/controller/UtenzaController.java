...
public class UtenzaController {
	...
	public Map<String, String> recuperaPassword(@RequestBody Map<String, String> data) {
		...
		validationUtil.validaEmailFormat(email);
		Utenza utenza = utenzaDAO.findByEmail(email).orElseThrow(() -> new InvalidFieldException("Email non trovata"));
		...
		if (utenza.getRuolo() == Ruolo.Amministratore && payload.containsKey("ruolo")) {
			...
			if ("Utente".equalsIgnoreCase(nuovoRuolo)) {
				throw new InvalidInputException(
						"Non è possibile rimuovere i privilegi di amministratore. Un amministratore può solo rimanere tale.");
			}
		}
	...
	public List<Map<String, Object>> getListaUtenti(@RequestHeader("Authorization") String token,
		...
		if (attivi != null) {
			utenti = utenti.stream()
				.filter(u -> u.getStato().equals(attivi))
				.collect(Collectors.toList());
		}
		
		// Filtro per nome (nome o cognome)
		if (nome != null && !nome.trim().isEmpty()) {
			String searchTerm = nome.toLowerCase().trim();
			utenti = utenti.stream()
				.filter(u -> u.getNome().toLowerCase().contains(searchTerm) || 
						    u.getCognome().toLowerCase().contains(searchTerm))
				.collect(Collectors.toList());
		}
